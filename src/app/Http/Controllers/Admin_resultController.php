...
class Admin_resultController extends Controller
{
    ...
    public function results_detail($quiz_id)  //BY Regions
    {
        ...
        if ($result_interpretations->count()>0) 
        {
            ...
            foreach ($result_interpretations as $interpret) 
            {
                ...
                $select_result = DB::select('SELECT COUNT(*) AS cnt FROM respondents
                    ...
                    AND respondent_results.scope <= ?;', [$quiz_id,$interpret->from,$interpret->to]);
                $ranges[$i]['count'] = $select_result[0]->cnt;
                $i++;
            }
        } else { $ranges = null;}

        $all_count = Respondent_result::where('quiz_id', $quiz_id)->count();
        //dd($all_count);
        $all_ranges = 0;
        ...
        {
            $byregion[$ri]['region'] = $region->name;
            $byregion[$ri]['region_id'] = $region->id;
            $respondents = $region->respondent()->get();
            ...
            foreach($respondents as $respondent)
            {
                ...
                $count += $results->count();
                $sum += $results->sum('scope');//$results->avg('scope');
            }

            $byregion[$ri]['count'] =  $count;
            $byregion[$ri]['sum'] =  $sum;
            $byregion[$ri]['contingent'] =  Scholler_count::where('region_id', $region->id)->get()->sum('count');
            //dd($byregion[$ri]['contingent']);
            $rgi = 0;
            if ((isset($ranges)) AND (count($ranges) > 0))
            {
                foreach($ranges as $range)
                {
                    $byregion[$ri]['ranges'][$rgi]['range'] = $range;
                    $select_result = DB::select('SELECT COUNT(*) AS cnt FROM respondents
                    INNER JOIN respondent_results ON respondent_results.respondent_id = respondents.id
                    WHERE respondents.region_id = ? 
                    AND respondent_results.quiz_id = ? 
                    AND respondent_results.scope >= ? 
                    AND respondent_results.scope <= ?;', [$region->id,$quiz_id,$range['from'],$range['to']]);
                    //dd($select_result[0]->cnt);
                    $byregion[$ri]['ranges'][$rgi]['count'] = $select_result[0]->cnt;
                    $rgi++;
                }              
            }

            if ($byregion[$ri]['count'] != 0)
                {
                    $byregion[$ri]['avg'] =  round($byregion[$ri]['sum'] /$byregion[$ri]['count'], 2);
                } else {
                    $byregion[$ri]['avg'] = 0;
                }
            $ri++;
        ...
        $reg = Region::first();
        $resp = $reg ->respondent()->get()->first();

        $quiz = Quiz::find($quiz_id);
        $iterprets = $quiz->result_interpretation()->get();

        $data = [
            'quiz' => $quiz,
            'regions' => $byregion,
            'interprets' => $iterprets,
            'all_count'  => $all_count,
            'all_ranges' => $all_ranges,
            'ranges'     => $ranges,
        ];
        return view('admin.results-detail', $data);
    }
    ...
    public function results_school_detail($school_id, $quiz_id)    // By grades
    {
        ...
        return view('admin.results-school-detail', $data);
    }    



    public function results_grade_detail($grade, $quiz_id, $school_id)    
    {
        $school = School::find($school_id);
        $quiz = Quiz::find($quiz_id);
        if ((!$quiz)OR(!$school))
        ...
        return view('admin.results-grade-detail', $data);
    }  
    ...
    public function results_answers($result_id, $quiz_id)
    {
        ...
        return view('admin.results-answers', $data);
        dd($questions);
        dd($result);
    }
